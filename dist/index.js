...
async function setEventOnCalendar(summary, start, end, calendarId) {
...
async function checkAndSetEvent(calendarId) {
    ...
    for (let i = 1; i <= days; i++) {
        ...
        _today.setDate(i);
        const dayLunar = getLunarDate(_today.getDate(), _today.getMonth() + 1, _today.getFullYear());
...
app.get('/', (c) => {
    const { CALENDAR_ID } = env(c);
    if (job === null) {
        job = new CronJob('0 0 0 1 * *', // cronTime
        // '10 * * * * *',
        function () {
